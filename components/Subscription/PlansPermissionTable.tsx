...
interface RawPlanPermission {
  id: string; // Tani waa ID-ga gaarka u ah xiriirka (Primary Key)
  planId: string;
  planName: string;
  permissionId: number;
...
interface GroupedPlanPermission {
  planId: string;
  planName: string;
  // Waxaan halkan ku daray 'permissionAssignmentIds' si aan u tirtirno
...
export default function PlanPermissionTable() {
        ...
        apiResponse.forEach((item) => {
          if (!grouped[item.planId]) {
            grouped[item.planId] = {
              ...
              permissionAssignmentIds: [], // Kaydi ID-yada xiriirka halkan
              permissions: [],
            };
          }
          // Ku dar ID-ga dhabta ah ee xariiq kasta (UUID)
  ...
  const confirmDelete = async () => {
    ...
    setDeleting(true);
    try {
      // Halkan waxaan u gudbinaynaa ID-ga gaarka ah (ama liiska ID-yada)
      // Haddii API-gu hal ID rabo: selectedItem.permissionAssignmentIds[0]
      // Haddii uu liis rabo, waxaad u gudbin kartaa dhammaan.
