...
interface Props {
  open: boolean;
  mode: "add" | "edit";
  initialData?: PermissionFormData; // Kani waa inuu u dhigmaa interface-ka sare
  onClose: () => void;
...
export default function PermissionFormModal({ open, mode, initialData, onClose, onSubmit }: Props) {
  ...
  const update = (k: keyof PermissionFormData, v: string) => {
    setForm((p) => ({ ...p, [k]: v }));
  ...
  const validate = () => {
    ...
    if (!form.keyName.trim()) e.keyName = "Key Name is required";
      ...
      >
        ...
        <div className="p-6 space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <Field label="Permission Name" required error={errors.name}>
              <Input value={form.name} onChange={(e) => update("name", e.target.value)} placeholder="Full Name" />
            </Field>

            <Field label="Key Name" required error={errors.keyName}>
              <Input value={form.keyName} onChange={(e) => update("keyName", e.target.value)} placeholder="Key Name" />
            </Field>
