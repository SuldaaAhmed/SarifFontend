...
interface Props {
  open: boolean;
  mode: "add" | "edit";
  initialData?: ModuleFormData; // Kani waa inuu u dhigmaa interface-ka sare
  onClose: () => void;
...
export default function ModuleFormModal({ open, mode, initialData, onClose, onSubmit }: Props) {
  ...
  const update = (k: keyof ModuleFormData, v: string) => {
    setForm((p) => ({ ...p, [k]: v }));
      ...
      >
        ...
        <div className="p-6 space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-1 gap-4">
            <Field label="Module Name" required error={errors.name}>
              <Input value={form.name} onChange={(e) => update("name", e.target.value)} placeholder="Module Name" />
            </Field>
