...
import { ModuleFormData } from "./ModuleFormModal";
...
interface Props {
  open: boolean;
  mode: "add" | "edit";
  initialData?: RoleFormData; // Kani waa inuu u dhigmaa interface-ka sare
  onClose: () => void;
...
export default function RoleFormModal({ open, mode, initialData, onClose, onSubmit }: Props) {
  ...
  useEffect(() => {
    if (open) {
      if (mode === "edit" && initialData) {
        ...
        setForm({
          ...initialData,
          roleName: initialData.roleName ?? "", // Haddii roleName uu null yahay, ka dhig ""
          description: initialData.description ?? "" // Haddii description uu null yahay, ka dhig ""
  ...
  const update = (k: keyof RoleFormData, v: string) => {
    setForm((p) => ({ ...p, [k]: v }));
        ...
        <div className="p-6 space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-1 gap-4">
            <Field label="Role Name" required error={errors.roleName}>
              <Input value={form.roleName} onChange={(e) => update("roleName", e.target.value)} placeholder="Role Name" />
            </Field>
