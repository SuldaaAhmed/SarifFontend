...
interface Props {
  open: boolean;
  mode: "add" | "edit";
  initialData?: AgencyFormData; // Kani waa inuu u dhigmaa interface-ka sare
  onClose: () => void;
...
export default function AgencyFormModal({ open, mode, initialData, onClose, onSubmit }: Props) {
  ...
  const update = (k: keyof AgencyFormData, v: string) => {
    setForm((p) => ({ ...p, [k]: v }));
      ...
      >
        <div className="relative p-6 border-b border-gray-100 dark:border-gray-800 text-center">
          <h3 className="text-lg font-semibold text-gray-900 dark:text-white">
            {mode === "add" ? "Add New Agency" : "Edit Agency Profile"}
          </h3>
          <button 
            onClick={onClose} 
